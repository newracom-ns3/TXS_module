...
void
WifiTxParameters::ClearInfo()
{
    m_info.clear();
}

void
WifiTxParameters::SetInfo(const PsduInfoMap& info)
{
    m_info = info;
}

WifiTxParameters::PsduInfoMap
WifiTxParameters::GetInfo() const
{
    return m_info;
}

void
WifiTxParameters::SetInfo(Mac48Address address, const PsduInfo* PsduInfo)
{
    auto infoIt = m_info.find(address);

    if (infoIt == m_info.end())
    {
        // Insert the info about the given frame
        m_info.emplace(address, *PsduInfo);
    }
    return;
}
