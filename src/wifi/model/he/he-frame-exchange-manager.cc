...
HeFrameExchangeManager::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::HeFrameExchangeManager")
            ...
            .SetGroupName("Wifi")
            .AddTraceSource("NumberofSendMuRts",
                            "The total number of sending MU-RTS",
                            MakeTraceSourceAccessor(&HeFrameExchangeManager::m_sendMuRtsTrace),
                            "ns3::TracedValueCallback::NumberofSendMuRts");
    return tid;
}
...
HeFrameExchangeManager::GetMuScheduler() const
{
...
HeFrameExchangeManager::SendMuRts(const WifiTxParameters& txParams)
{
    ...
    // Tracing send MU-RTS @cm.lee
    // std::cout << m_sendMuRtsTrace << "\n";
    m_sendMuRtsTrace++;
}
...
HeFrameExchangeManager::UpdateNav(Ptr<const WifiPsdu> psdu, const WifiTxVector& txVector)
{
    ...
    if (intraBssNavEnd > m_intraBssNavEnd)
    {
        ...
        if (psdu->GetHeader(0).IsRts() && !psdu->GetHeader(0).IsFromDs())
        {
            WifiTxVector ctsTxVector =
