...
RrMultiUserScheduler::TrySendingDlMuPpdu()
{
...
#if NRC_DEBUG == true
                    std::cout
                        << mpdu->GetHeader().GetAddr1() << " is "
                        << m_apMac->GetFrameExchangeManager(m_linkId)->GetProtectedStas().count(
    ...
    {
        std::cout << "Protected method: " << m_txParams.m_protection->method << std::endl;
    }
#endif
...
RrMultiUserScheduler::ComputeDlMuInfo()
{
    ...
    if (m_candidates.size() > 1)
    {
        for (const auto& candidate : m_candidates)
        {
            std::cout << candidate.second->GetHeader().GetAddr1() << " is "
                      << m_apMac->GetFrameExchangeManager(m_linkId)->GetProtectedStas().count(
                             candidate.second->GetHeader().GetAddr1())
                      << std::endl;
        }

        std::cout << "Protected method: " << dlMuInfo.txParams.m_protection->method << std::endl;
        std::cout << "PsduInfoMap Size: " << dlMuInfo.txParams.GetPsduInfoMap().size() << std::endl;
    }
#endif

    WifiTxParameters backupTxParams = dlMuInfo.txParams;
    // std::unique_ptr<WifiProtection> backupProtection = std::move(dlMuInfo.txParams.m_protection);
    std::unique_ptr<WifiProtection> backupProtection;
    std::swap(backupProtection, dlMuInfo.txParams.m_protection);
    dlMuInfo.txParams.ClearInfo();
    for (const auto& candidate : m_candidates)
    {
        ...
        {
            Mac48Address receiver = candidate.first->address;
            dlMuInfo.txParams.SetInfo(receiver, backupTxParams.GetPsduInfo(receiver));
    ...
    for (const auto& psdu : dlMuInfo.psduMap)
    {
        psduMap.emplace(psdu.first, psdu.second);
    }

    Time txDuration =
        m_apMac->GetWifiPhy()->CalculateTxDuration(psduMap,
                                                   dlMuInfo.txParams.m_txVector,
