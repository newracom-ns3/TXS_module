...
NS_LOG_COMPONENT_DEFINE("nrc-uora-example");
...
main(int argc, char* argv[])
{
    ...
    bool udp{true};
    bool downlink{true};
    bool enableUlOfdma{true}; // uora-example enable
    bool enableBsrp{false};
    std::size_t nStations{4};
    std::string dlAckSeqType{"MU-BAR"}; // MU-BAR은 현재 리시버 주소 겹치는 문제가 있음.
    ...
    for (int mcs = minMcs; mcs <= maxMcs; mcs++)
    {
             ...
             channelWidth++) // MHz
        {
            ...
            if (udp)
            {
                ...
                for (std::size_t i = 0; i < nStations; i++)
                {
                    ...
                    if (packetQosSupport)
                    {
                        ...
                        apClient.SetAttribute("DataRate", StringValue("100Mbps"));
                        ...
                        staClient.SetAttribute(
                            "DataRate",
                            StringValue("25Mbps")); // Maybe we need to make this slow
            ...
            */
            ...
            std::string headlineTx = "Scenario UORA: Tx packet bytes";
            std::string headlineRx = "Scenario UORA: Rx packet bytes";
            ...
            std::string filePathTx = "newracom-data/UORA/tx-packet-" + std::to_string(simTrial);
            std::string filePathRx = "newracom-data/UORA/rx-packet-" + std::to_string(simTrial);
