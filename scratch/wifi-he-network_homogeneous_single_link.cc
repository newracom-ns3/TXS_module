...
main(int argc, char* argv[])
{
    ...
    for (int mcs = minMcs; mcs <= maxMcs; mcs++)
    {
        ...
        uint8_t maxChannelWidth = frequency == 2.4 ? 40 : 80;
        // for (int channelWidth = 20; channelWidth <= maxChannelWidth;) // MHz
        for (int channelWidth = frequency == 2.4 ? 20 : 80; channelWidth <= maxChannelWidth;) // MHz
        {
            // for (int gi = 3200; gi >= 800;) // Nanoseconds
            for (int gi = 1600; gi >= 1600;) // Nanoseconds
            {
                if (!udp)
                {
                ...
                for (std::size_t i = 0; i < nStations; i++)
                {
                    ...
                    std::cout << "추가된 클라이언트노드: " << clientNodes.Get(i) << "\n";
                }

                std::cout << "clientNodes 개수: " << clientNodes.GetN() << "\n";
                std::cout << "serverNodes 개수: " << wifiStaNodes.GetN() << "\n";
                ...
                */
                ...
                std::string headline = "Tx packet bytes";
                fileHelper.SetHeading(headline);

                // Configure the file to be written, and the formatting of output data.
                std::string filePath =
                    "newracom-data/data/single-tx-packet-" + std::to_string(simTrial);
                simTrial++;
                std::cout << filePath << "\n";
                ...
                fileHelper.WriteProbe(probeType, tracePath_tx, "OutputBytes");
